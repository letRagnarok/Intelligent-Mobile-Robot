    ...
    for k in range(1000000):
        ...
        while not done:
            ...
            if t % update_interval == 0:
                ...
                if total_score / print_interval > best_score and t % 100 == 0:
                    q_filename = f"./train/q/mario_q_{best_score}.pth"
                    q_target_filename = f"./train/q_target/mario_q_target_{best_score}.pth"
                    torch.save(q.state_dict(), q_filename)
                    torch.save(q_target.state_dict(), q_target_filename)
                    best_score = total_score / print_interval
...
if __name__ == "__main__":
    ...
    q.load_state_dict(torch.load(ckpt_path1, map_location=torch.device(device)))

    q_target = model(n_frame, env.action_space.n, device).to(device)
    q_target.load_state_dict(torch.load(ckpt_path2, map_location=torch.device(device)))
